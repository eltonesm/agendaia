package com.simboraagendar.platform.security;
...
/**
 * Login isolado do operador da plataforma — quem opera o SimboraAgendar, não é
...
public class OperatorSecurityConfig {
    ...
    @Bean
    UserDetailsService operatorUserDetailsService(
            @Value("${simboraagendar.operador.username}") String username,
            @Value("${simboraagendar.operador.password-hash}") String passwordHash) {
        var operador =
