...
class ObservabilidadeIT {
    ...
    void e2e3PrometheusExigeCredencialEExpoeMetricas() throws Exception {
        ...
        var antes = mockMvc.perform(get("/actuator/prometheus").with(httpBasic("metrics", "metrics-dev-only")))
                ...
                .getResponse()
                .getContentAsString();
        var criadoAntes = extrairContador(antes, "agendaia_appointments_booked_total");
        var canceladoAntes = extrairContador(antes, "agendaia_appointments_cancelled_total");
        var conflitoAntes = extrairContador(antes, "agendaia_appointments_slot_conflict_total");
        ...
        var depois = mockMvc.perform(get("/actuator/prometheus").with(httpBasic("metrics", "metrics-dev-only")))
                ...
                .getResponse()
                .getContentAsString();
        var criadoDepois = extrairContador(depois, "agendaia_appointments_booked_total");
        var conflitoDepois = extrairContador(depois, "agendaia_appointments_slot_conflict_total");
